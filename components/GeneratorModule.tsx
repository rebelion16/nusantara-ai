...
import { sendImageToTelegram, getTelegramSettings, saveTelegramSettings, testTelegramConnection, isTelegramConfigured } from '../services/telegramService';
import { ModuleId } from '../types';
import { ErrorPopup } from './ErrorPopup';
import { X, Send, Settings, MessageCircle } from 'lucide-react';
...
export const GeneratorModule: React.FC<GeneratorModuleProps> = ({
  ...
  useEffect(() => {
    setTelegramConfigured(isTelegramConfigured());
  ...
  // Telegram Handlers
  const handleSendToTelegram = async (imageUrl: string) => {
    if (!imageUrl) return;
    ...
    try {
      const result = await sendImageToTelegram(imageUrl, `🎨 Generated by Nusantara AI Studio\n📝 ${prompt.slice(0, 200)}...`);
  ...
  const handleSaveTelegramSettings = async () => {
    ...
    saveTelegramSettings(telegramBotToken.trim(), telegramChatId.trim());
  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          <div className={`bg-gray-100 dark:bg-gray-800 rounded-2xl p-4 min-h-[400px] border border-gray-200 dark:border-gray-700 relative overflow-hidden group ${!generatedImage && !batchResults.length ? 'flex items-center justify-center' : ''}`}>
            {batchResults.length > 0 && isBatchMode ? (
              ...
              <div className="relative w-full h-full flex flex-col items-center justify-center animate-fade-in">
                ...
                {/* Action Buttons - Download & Telegram */}
                <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
                  {/* Download Button */}
                  <a
                    href={generatedImage}
                    download={`nusantara-ai-${Date.now()}.png`}
                    className="px-5 py-2.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-full font-medium shadow-xl hover:scale-105 transition-transform flex items-center gap-2 border border-gray-200 dark:border-gray-600"
                  >
                    <span>Unduh HD</span>
                  ...
                  <button
                    onClick={() => handleSendToTelegram(generatedImage)}
                    disabled={telegramSending}
                    className={`px-5 py-2.5 bg-[#0088cc] hover:bg-[#006699] text-white rounded-full font-medium shadow-xl hover:scale-105 transition-transform flex items-center gap-2 ${telegramSending ? 'opacity-50 cursor-wait' : ''}`}
                  >
                    {telegramSending ? (
                      <>
                  ...
                  <button
                    ...
                    className="p-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full shadow hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
      ...
      {showTelegramSettings && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowTelegramSettings(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scale-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 bg-[#0088cc] rounded-full flex items-center justify-center">
                  <MessageCircle size={20} className="text-white" />
                </div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Pengaturan Telegram</h3>
              </div>
              <button onClick={() => setShowTelegramSettings(false)} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-lg text-sm text-blue-800 dark:text-blue-200">
                ...
                <ol className="list-decimal list-inside space-y-1 text-xs">
                  ...
                  <li>Chat bot Anda dan kirim /start</li>
                  <li>Buka @userinfobot untuk dapat Chat ID</li>
                </ol>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-[#0088cc] focus:border-transparent"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors text-sm"
                >
                  🔗 Test Koneksi
                </button>
                <button
                  onClick={handleSaveTelegramSettings}
                  className="flex-1 px-4 py-2.5 bg-[#0088cc] hover:bg-[#006699] text-white rounded-lg font-medium transition-colors text-sm"
                >
                  💾 Simpan
                </button>
              </div>
              ...
              {telegramConfigured && (
                <p className="text-center text-xs text-green-600 dark:text-green-400">✅ Telegram sudah terkonfigurasi</p>
              )}
            </div>
          </div>
        </div>
