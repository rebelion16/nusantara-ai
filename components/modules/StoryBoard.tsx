...
export const StoryBoardModule: React.FC<StoryBoardProps> = ({ initialImage }) => {
  ...
  const constructPrompt = (scene: Scene) => {
    ...
    - The character in this scene MUST BE 100% IDENTICAL to the Reference Image provided (Face, Hair, Outfit, Accessories).
    - **FACE:** Do NOT change facial structure. It must look exactly like the reference person.
    - **OUTFIT:** Keep the outfit exactly the same unless the Action specifically says otherwise.
    - If the reference holds a weapon/item, keep it unless specified otherwise.
    ...
    Make it a high-quality, cinematic shot.
    `;
  };
  ...
  const handleGenerateVideo = async (id: number) => {
    ...
    try {
      ...
      prompt += `\nCharacter appearance must be consistent with the starting image. High quality motion, 8k resolution.`;
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                   ...
                                   <button 
                                      ...
                                      className={`text-[9px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 border transition-colors ${
                                        scene.includeDialogueInVideo 
